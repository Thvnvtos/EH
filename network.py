...
class NetCNN2D_CSP(keras.Model):
    def __init__(self, n_classes, **kwargs):
        ...
        self.cnn1 = keras.Sequential(
            [
                keras.layers.Conv2D(filters=8, kernel_size=(7,7), strides=(7,7), padding='same'),
                keras.layers.BatchNormalization(),
                keras.layers.ReLU(max_value=1000.0),
        ...
        self.cnn2 = keras.Sequential(
            [
                keras.layers.Conv2D(filters=16, kernel_size=(5,5), strides=(5,5), padding='same'),
                keras.layers.BatchNormalization(),
                keras.layers.ReLU(max_value=1000.0)
            ], name='cnn2')
...
def build_functional_cnn2D(n_classes, input_shape):
    ...
    # cnn1
    x = tf.keras.layers.Conv2D(filters=8, kernel_size=(7, 7), strides=(7, 7), padding='same')(inputs)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU(max_value=1000.0)(x)

    # cnn2
    x = tf.keras.layers.Conv2D(filters=16, kernel_size=(5, 5), strides=(5, 5), padding='same')(x)
    x = tf.keras.layers.BatchNormalization()(x)
    x = tf.keras.layers.ReLU(max_value=1000.0)(x)
